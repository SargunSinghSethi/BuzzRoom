...
export default async function handler(
    ...
    try {
        ...
        if(!canModify) return res.status(401).json({ error: "Unauthorized - Cannot delete this message"});
        ...
        if(req.method === "PATCH") {
            if(!isMessageOwner) return res.status(401).json({ error: "Unauthorized Access - not message owner"});
            message = await db.message.update({
                where: {
                    id: messageId as string,
                },
